surv_stoEM_ipw <- function(data, zetat, zetaz, B = 100, theta = 0.5){
  ...
  for (j in 1:B){
    ...
    t1.ipw = survreg(Surv(t, d) ~ Z, weights = ipw, robust = TRUE, dist = "weibull")
    mu1 <- predict(t1.ipw, newdata=data.frame(Z=c(rep(1,n))), type="lp")
    mu0 <- predict(t1.ipw, newdata=data.frame(Z=c(rep(0,n))), type="lp")
    sigma <-  t1.ipw$scale
    S1 <- exp(-exp((log(t_pred)-mu1)/sigma))
    S0 <- exp(-exp((log(t_pred)-mu0)/sigma))
