SimulateU_surv <- function(t, d, z, x, zetat, zetaz, theta = 0.5, iter = 20, weights = NULL, offset = FALSE){
  ...
  for(j in 1:iter){
    if(offset){
      # Fit Weibull AFT with offset for U
      U.fit1 = survreg(Surv(t, d) ~ z + x + offset(U * zetat), weights = weights, dist = "weibull")
      t.coef1 = c(U.fit1$coef, zetat)  #zetat as fixed effect
      
      # Fit treatment model with offset for U
      z.fit = glm(z ~ x, family = binomial(link = "probit"), 
                  offset = zetaz * U,
                  control = glm.control(epsilon = 1e-6, maxit = 50))
      z.coef = c(z.fit$coef, zetaz)  # append zetaz as fixed effect
    } else {
      #fit time to event
    ...
    if(offset){
      ...
      Xmat <- model.matrix(~ z + x)
      
      # U = 1
      Xmat1 <- cbind(Xmat, U = 1)
      lp1 <- Xmat1 %*% matrix(t.coef1, ncol = 1)
      lambda1 <- exp(-lp1 / sigma)
      
      # U = 0
      Xmat0 <- cbind(Xmat, U = 0)
      lp0 <- Xmat0 %*% matrix(t.coef1, ncol = 1)
      lambda0 <- exp(-lp0 / sigma)
      
      # Treatment model matrix
      Zmat1 <- cbind(1, x, 1)
      Zmat0 <- cbind(1, x, 0)
      
      # Compute posterior probabilities
      ptzu1 <- (1 - pnorm(Zmat1 %*% matrix(z.coef, ncol = 1)))^(1 - z) *
        pnorm(Zmat1 %*% matrix(z.coef, ncol = 1))^z * theta *
        (v * lambda1^v * t^(v - 1))^d *
        exp(-lambda1^v * t^v)
      
      ptzu0 <- (1 - pnorm(Zmat0 %*% matrix(z.coef, ncol = 1)))^(1 - z) *
        pnorm(Zmat0 %*% matrix(z.coef, ncol = 1))^z * (1 - theta) *
        (v * lambda0^v * t^(v - 1))^d *
        exp(-lambda0^v * t^v)
    }
    else{
