...
for (i in 1:n_sim) {
  ...
  #T~Weibull(lamda=exp(-lp/sigma), v=1/sigma)
  sigma <- 0.5
  ...
  source("SimulateU_surv_wb.R")
  source("surv_stoEM_ipw_wb.R")
  ...
  for(b in 1:nboot){
    ...
    spce_huang.b[b] <- surv_stoEM_ipw(data=data.boot, zetat=-0.8, zetaz=-0.8, B = 50, theta = 0.5)$spce
